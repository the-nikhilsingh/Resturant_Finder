...
if (isset($_POST["btn"])) {
  $test = $_POST["btn"];
  $sql= "INSERT into `current_order` (`cust_id`,`b_id`,`item_id`,`TimeStamp`) VALUES($cust_id,$branch_id,$test,now())";
...
while($id1 <= $limit){
  ...
  $b = $fetch["price"];
  //echo gettype($a);
  echo '<div style="margin-left:20%;columns: 300px 3;">';
...
<nav class="cart">
  ...
  $sql = "SELECT desserts, desserts.item_id, current_order.order_id from `desserts`,`current_order`  where desserts.b_id = $branch_id and cust_id= $cust_id and current_order.item_id= desserts.item_id and current_order.b_id = desserts.b_id;";
  $result = $conn->query($sql);
  if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
      echo $row['desserts'].'<br>'.$row['item_id'].''.$row['order_id'].'<input type="hidden" name="des_name" value="'.$row['desserts'].'" /><input type="hidden" name="ord_id" value="'.$row['order_id'].'" /><input type="hidden" name="itm_id" value="'.$row['item_id'].'" /><button type="submit" name="submit" value="submit" />remove from cart</button>';
    }
  }
  else{
  ...
  ?>
  <button ><a href="confirm.php">confirm your ORDER</a></button><br>
</nav>
