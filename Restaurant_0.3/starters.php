...
$query = "SELECT count(*) FROM starters where b_id = $branch_id" ;
...
if (isset($_POST["btn"])) {
  $test = $_POST["btn"];
  $sql= "INSERT into `current_order` (`cust_id`,`b_id`,`item_id`,`TimeStamp`,`course`) VALUES($cust_id,$branch_id,$test,now(),'starters')";
...
echo '<form action="" method="post"><h2 style="margin-left:20%;">starters</h2>';
while($id1 <= $limit){
  $query = "SELECT * FROM starters where item_id = $id1 and b_id = $branch_id" ;
  $sth = $conn->query($query);
...
<nav class="cart">
  ...
  $sql = "SELECT starters, starters.item_id, current_order.order_id from `starters`,`current_order`  where starters.b_id = $branch_id and cust_id= $cust_id and current_order.item_id= starters.item_id and current_order.b_id = starters.b_id and course='starters';";
  $result = $conn->query($sql);
  if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
      echo '<br />'.$row['starters'].'<br><input type="hidden" name="des_name" value="'.$row['starters'].'" /><input type="hidden" name="ord_id" value="'.$row['order_id'].'" /><input type="hidden" name="itm_id" value="'.$row['item_id'].'" /><button type="submit" name="submit" value="submit" />remove from cart</button>';
    }
  }
  else{
  ...
  ?>
  <br><button ><a href="mains.php?a=<?php echo $branch_id ?> ">Continue</a></button><br>
</nav>
